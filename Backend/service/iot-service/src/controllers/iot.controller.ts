...
export class IoTController {
    ...
    private async getSimulatableDevices(deviceEui?: string) {
     ...
     */
    private async ingestReadingPayload(payload: any) {
        const { device_eui, salinity, temperature, ph, water_level, battery_voltage, timestamp, force_insert } = payload;
        const salinityValue = Number(salinity);
        if (!Number.isFinite(salinityValue)) {
            throw new Error('Invalid salinity value');
        }
        ...
        if (shouldInsert) {
            ...
            if (readingTimestamp) insertPayload.timestamp = readingTimestamp;
    ...
    private async runRealtimeSimulationStep(deviceEui?: string) {
        const devices = await this.getSimulatableDevices(deviceEui);
        this.simulationStatus.device_count = devices.length;
        if (devices.length === 0) return;

        const now = new Date();
        for (const device of devices) {
            const seed = this.hashString(String(device.device_eui || device.id));
            const reading = this.buildTrainLikeSimulatedReading(now, seed);
            await this.ingestReadingPayload({
        ...
        this.simulationStatus.last_tick_at = now.toISOString();
    }
    ...
    private stopSimulationLoop() {
        ...
        this.simulationStatus.active = false;
        this.simulationStatus.started_at = null;
    }

    async startRealtimeSimulation(request: any, reply: any) {
        try {
            const intervalMsRaw = Number(request.body?.interval_ms ?? 2000);
            const intervalMs = Number.isFinite(intervalMsRaw)
                ? Math.max(1000, Math.min(60000, Math.round(intervalMsRaw)))
                : 2000;

            await this.runRealtimeSimulationStep();
            this.stopSimulationLoop();

            this.simulationStatus.active = true;
            this.simulationStatus.interval_ms = intervalMs;
            this.simulationStatus.started_at = new Date().toISOString();

            this.simulationTimer = setInterval(() => {
                this.runRealtimeSimulationStep().catch((error) => {
                    logger.error(`Realtime simulation tick error: ${error.message}`);
                });
            }, intervalMs);

            return {
                success: true,
                message: 'Realtime simulation started.',
                data: this.simulationStatus,
            };
        } catch (error: any) {
            logger.error(`Start realtime simulation error: ${error.message}`);
            return reply.status(500).send({ success: false, message: error.message });
        }
    }

    async stopRealtimeSimulation(request: any, reply: any) {
        try {
            this.stopSimulationLoop();
            return {
                success: true,
                message: 'Realtime simulation stopped.',
                data: this.simulationStatus,
            };
        } catch (error: any) {
            logger.error(`Stop realtime simulation error: ${error.message}`);
            return reply.status(500).send({ success: false, message: error.message });
        }
    }

    async getRealtimeSimulationStatus(request: any, reply: any) {
        try {
            return {
                success: true,
                data: this.simulationStatus,
            ...
            logger.error(`Get realtime simulation status error: ${error.message}`);
            return reply.status(500).send({ success: false, message: error.message });
        }
    }
