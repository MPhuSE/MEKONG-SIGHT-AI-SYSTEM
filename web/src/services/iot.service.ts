...
export const iotService = {
    ...
    startRealtimeSimulation: async (intervalMs = 2000) => {
        const response = await api.post('/iot/simulate/start', {
            interval_ms: intervalMs,
        });
        return response.data;
    },

    stopRealtimeSimulation: async () => {
        const response = await api.post('/iot/simulate/stop');
        return response.data;
    },

    getRealtimeSimulationStatus: async () => {
        const response = await api.get('/iot/simulate/status');
        return response.data;
    },
};
